...
lib.getEngine(conf, cb)
  ...
  - Polls bot18.net for activation success (code status is "used", license_status set to "active")
  ...
  - The result is cached to disk in encrypted, packed form by the key:
      ~/.bot18/builds/{sha1(code + pubkey + request_ip + channel + current_engine_version)}.bot18
        ...
        mr.post('https://code.bot18.net/auth/' + conf.code, opts, function (err, resp, body) {
          ...
          if (resp.statusCode !== 200) {
            switch (resp.statusCode) {
              ...
              case 411:
                return cb(new Error('You specified an invalid channel. Channel should be one of "stable", "unstable", or "free".'))
              case 415:
